...
def grafica(persones):   
    dades = persones # Valors de les variables
    ...
    rects1 = ax1.bar(ind, dades, width, color='b') # Dades dels diagrames. Coordenades, Valors de les variables, Amplada i Color.
    ax1.set_ylabel('Número') # Llegenda de eix Y.
    ax1.set_xlabel('Nº de Personas') # Llegenda de eix Y.
    ax1.set_title('') # Titol del grafic
    ...
    ########################################## GUARDAR LA FIGURA #########################################################################
    nombrevar = str(date.datetime.now()) + '.png'
    nombrevar = nombrevar.replace(':','-')
    plt.savefig(nombrevar, bbox_inches='tight')
    plt.close(fig)
